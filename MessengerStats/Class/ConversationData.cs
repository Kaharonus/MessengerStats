...
namespace MessengerStats {
    public class ConversationData {
        ...
        static async Task<Conversation> ParseConversation(string dir, EventHandler conversationChange = null) {
            ...
            var enumerable = concat as Message[] ?? concat.ToArray();
            var firstOrLast = enumerable.Max(x => x.Time);
            conv.Value.LastMessage = enumerable.First(x => x.Time == firstOrLast);

            firstOrLast = enumerable.Min(x => x.Time);
            conv.Value.FirstMessage = enumerable.First(x => x.Time == firstOrLast);

            return conv.Value;
        }

        /// <summary>
        /// Assigns activity per week between 0-100 instead of 0-whatever
        /// </summary>
        private static void WeightActivity() {
            var max = Conversations.Max(x => x.ActivityPerWeek);
            foreach (var f in Conversations) {
                f.ActivityPerWeek = f.ActivityPerWeek * 100.0 / max;
            }
        }
